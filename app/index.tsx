...
export default function Index() {
  ...
  }, [authState, authLoading, modelsLoading, themeLoaded, profile?.onboardingCompleted]);
      ...
      {__DEV__ && (
        <View style={styles.debugContainer}>
          <Text style={[styles.debugText, { color: colors.secondary }]}>
            Auth: {authState} | Theme: {themeLoaded ? '✅' : '⏳'} | Models: {modelsLoading ? '⏳' : '✅'}
          </Text>
          {user && (
            <Text style={[styles.debugText, { color: colors.secondary }]}>
              Email verified: {user.emailVerified ? '✅' : '❌'} | Onboarding: {profile?.onboardingCompleted ? '✅' : '❌'}
            </Text>
          )}
        </View>
      )}
